...
function CreateSensorModal({ onClose, onCreated }) {
  const [form, setForm] = useState({
    name: '', type: '', manufacturer: '', manufacturing_date: ''
  })
  ...
  const validate = () => {
    ...
    if (new Date(form.manufacturing_date) > new Date())
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-gray-200 w-full max-w-md mx-4 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-base font-semibold text-gray-800">Nuevo Sensor</h2>
              <p className="text-xs text-gray-400">Registra un nuevo sensor en el sistema</p>
            </div>
          </div>
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          <div>
            ...
            <input type="text" name="name" placeholder="Ej: ST-104, SP-205"
              value={form.name} onChange={handleChange} className={inputClass} maxLength={100} />
          </div>
          ...
          <div>
            ...
            {form.type && (
              <p className="text-xs text-blue-500 mt-1">ℹ {TYPE_INFO[form.type].desc}</p>
            )}
          </div>

          <div>
            <label className={labelClass}>Fabricante</label>
            <input type="text" name="manufacturer" placeholder="Ej: Siemens, Honeywell, ABB"
              value={form.manufacturer} onChange={handleChange} className={inputClass} maxLength={100} />
          </div>

          <div>
            <label className={labelClass}>Fecha de fabricación</label>
            <input type="date" name="manufacturing_date"
              value={form.manufacturing_date} onChange={handleChange}
          ...
          {success && (
            <div className="bg-green-50 border border-green-200 text-green-600 rounded-lg px-3 py-2 text-sm flex items-center gap-2">
              <CheckCircle size={15} /> Sensor creado correctamente
            </div>
          )}
        </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
          ...
          <button onClick={handleSubmit} disabled={loading || success}
            ...
            <Cpu size={14} />
            {loading ? 'Creando...' : success ? 'Creado ✓' : 'Crear Sensor'}
          </button>
        </div>
      </div>
    </div>
